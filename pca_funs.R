## modified from Filip Kolar and
## https://github.com/pmonnahan/ArenosaPloidy/blob/master/adegenet_PCA_genomic_300.R
...
## plotting data with vcfR
makepca0 <- function(vcfRpca){
    data <- vcfRpca
    pd <- data.frame(sample.id = rownames(data$scores),
                     pop = as.factor(substr(rownames(data$scores),1,3)),
                     ev1 = unname(data$scores[,1]),
                     ev2 = unname(data$scores[,2]),
                     ev3 = unname(data$scores[,3]),
                     ev4 = unname(data$scores[,4]),
                     ev5 = unname(data$scores[,5]),
                     stringsAsFactors = F)
}
